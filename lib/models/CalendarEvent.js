    ...
    try {
      ...
      try {
        if (isAllDay) {
          ...
          // (the day after the event ends), so we need to store both dates correctly
          if (eventData.end && eventData.end.date) {
            endTime = eventData.end.date;
            defaultLogger.info(`All-day event end date: ${endTime}`);
          } else {
          ...
          // For timed events, use ISO format: YYYY-MM-DDTHH:mm consistently
          const startDate = new Date(eventData.start.dateTime);
          // Store in ISO format but truncate to minutes precision for consistency
          startTime = startDate.toISOString().split('.')[0].substring(0, 16);
          defaultLogger.info(`Timed event start: ${startTime}`);
          
          if (eventData.end && eventData.end.dateTime) {
            const endDate = new Date(eventData.end.dateTime);
            // Store in ISO format but truncate to minutes precision for consistency
            endTime = endDate.toISOString().split('.')[0].substring(0, 16);
            defaultLogger.info(`Timed event end: ${endTime}`);
            
            // Validate end time is after start time
            if (endDate <= startDate) {
              // Default to 1 hour duration if end time is invalid
              endTime = new Date(startDate.getTime() + 60 * 60 * 1000).toISOString().split('.')[0].substring(0, 16);
        ...
        if (isAllDay && eventData.start?.date) {
          ...
          startTime = eventData.start?.dateTime?.substring(0, 16) || null;
          endTime = eventData.end?.dateTime?.substring(0, 16) || startTime;
        }
      }
