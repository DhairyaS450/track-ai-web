...
const REDIRECT_URI = process.env.REDIRECT_URI || 'https://tidaltasks.app/settings';
...
exports.connectCalendar = async (req, res) => {
  try {
    const { code, uid: bodyUid } = req.body;
    // Use req.user.uid if available, otherwise fall back to the uid from the request body
    const uid = req.user?.uid || bodyUid;

    defaultLogger.info(`Request headers: ${JSON.stringify(req.headers)}`);
    ...
    if (!uid) {
      defaultLogger.error('No user ID provided');
...
exports.syncGoogleEvents = async (req, res) => {
  try {
    ...
    // Fetch events from last three days to next week
...
exports.updateGoogleEvent = async (req, res) => {
  try {
    ...
    defaultLogger.info(`Updating event ${eventId} for user ${uid} with data: ${JSON.stringify(updates)}`);
    
    // Validate the update data
    if (!updates || (updates.updates && Object.keys(updates.updates).length === 0)) {
...
exports.updateGoogleTask = async (req, res) => {
  try {
    ...
    defaultLogger.info(`Updating task ${taskId} for user ${uid} with data: ${JSON.stringify(updates)}`);
