...
exports.autoScheduleStudySessions = async (req, res) => {
  try {
    ...
    if (!subject || !totalDurationRequired || !deadline) {
      defaultLogger.error(`Missing required parameters for auto-scheduling`);
    ...
    // Fetch user's profile for preferences
    ...
    // Get school hours
    const schoolStartTime = userData?.schoolStartTime || '08:00';
    const schoolEndTime = userData?.schoolEndTime || '15:00';
...
{
  "subject": "${subject}",
  "description": "Auto-scheduled study session for: ${description || subject}",
...
IMPORTANT: The sum of all "duration" values across all sessions must equal ${totalDurationRequired}.
All "scheduledFor" times must be in the future, between now and ${deadline}.
`;

    // Send request to Gemini API
    defaultLogger.info(`Sending auto-schedule request to Gemini API`);
    let response;
    try {
      ...
      if (!response || typeof response !== 'string') {
        defaultLogger.error(`Invalid response received from Gemini API: ${JSON.stringify(response)}`);
      ...
      defaultLogger.error(`Error calling Gemini API: ${llmError.message}`);
      return res.status(500).json({ 
    ...
    defaultLogger.info(`Parsing Gemini API response`);
    let parsedResult;
    try {
      parsedResult = parseAutoScheduleResponse(response);
      defaultLogger.info(`Parse result: ${JSON.stringify(parsedResult)}`);
    } catch (parseError) {
      defaultLogger.error(`Error parsing LLM response: ${parseError.message}`);
      defaultLogger.error(`Raw response was: ${response}`);
      return res.status(500).json({ 
